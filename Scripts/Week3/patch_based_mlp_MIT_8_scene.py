...
config = "psize_64_bsize_8_layers_4096_1024_8"
MODEL_FNAME = f'/home/group05/work/models/patch_based_mlp/{config}patch_based_mlp.h5'
...
def build_mlp(input_size=PATCH_SIZE,phase='TRAIN'):
  ...
  if phase=='TEST':
    model.add(Dense(units=8, activation='linear',name='fourth')) # In test phase we softmax the average output over the image patches
  else:
    model.add(Dense(units=8, activation='softmax',name='fourth'))
  return model
...
if not os.path.exists(MODEL_FNAME):
  ...
  model.fit(
          ...
          validation_steps=8070 // BATCH_SIZE,verbose=1)
...
for class_dir in os.listdir(directory):
    ...
    for imname in os.listdir(os.path.join(directory,class_dir)):
      ...
      patches = image.extract_patches_2d(np.array(im), (PATCH_SIZE, PATCH_SIZE), max_patches=int(np.asarray(im).shape[0]/PATCH_SIZE)*int(np.asarray(im).shape[1]/PATCH_SIZE))
